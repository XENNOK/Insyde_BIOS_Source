  ...
  )
{
  ...
  while (!IsNull (&Form->StatementListHead, Link)) {
    ...
    if (FirstSelectableStatement == NULL && IsSelectable (Statement, 0)) {
  ...
  ASSERT (Private->FB.CurrentQ != NULL);
  ...
  )
{
  ...
  //
  // SCU / SecureBootMgr don't care NvUpdateRequired flag
  //
  if (gBrowserSettingScope == FormSetLevel &&
      (CompareGuid (gCurrentSelection->FormSet->ClassGuid, &mScuFormSetGuid) ||
       CompareGuid (Selection->FormSet->ClassGuid, &mSecureBootMgrFormSetGuid)) && Scope == FormSetLevel) {
  ...
  if (ParentMenu != NULL) {
    ...
    } else if (CompareGuid (gCurrentSelection->FormSet->ClassGuid, &mScuFormSetGuid)) {
