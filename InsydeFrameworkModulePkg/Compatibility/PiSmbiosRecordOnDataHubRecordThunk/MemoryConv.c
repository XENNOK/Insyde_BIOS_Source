  ...
  )
{
  ...
  MemoryDeviceErrorInfoLink = (EFI_INTER_LINK_DATA*)(PhyMemArray + 1);
  ...
  *(UINT8 *) ((UINT8 *) (StructureNode->Structure) + OFFSET_OF (SMBIOS_TABLE_TYPE16, MemoryErrorCorrection))  = (UINT8) (PhyMemArray->MemoryErrorCorrection);
  ...
  //
  // Memory Device Error Information (Type 18) Link
  //
  SmbiosFldInterLink (
    StructureNode,
    0x0b,
    18, // SMBIOS type 18
    MemoryDeviceErrorInfoLink,
...
{
  EFI_MEMORY_ARRAY_LINK_PLUS       *MemDevice;
  UINT64                           MemoryDeviceSize;
  BOOLEAN                          MemoryDeviceSizeUnitMega;
  UINT16                           MemoryDeviceSpeed;
  UINT16                           MemoryDeviceExponent;
  UINT16                           Test16;
  UINT64                           MemoryExtendedSize;
  
  MemDevice = (EFI_MEMORY_ARRAY_LINK_PLUS *) RecordData;
  MemoryDeviceSpeed  = 0;
  MemoryDeviceExponent = 0;
  MemoryExtendedSize = 0;
  ...
  SmbiosFldString (
    ...
    &(MemDevice->MemoryArrayLink.MemoryDeviceLocator),
    sizeof (STRING_REF)
    );
  ...
  SmbiosFldString (
    ...
    &(MemDevice->MemoryArrayLink.MemoryBankLocator),
    sizeof (STRING_REF)
    );
  ...
  SmbiosFldString (
    ...
    &(MemDevice->MemoryArrayLink.MemoryManufacturer),
    sizeof (STRING_REF)
    );
  ...
  SmbiosFldString (
    ...
    &(MemDevice->MemoryArrayLink.MemorySerialNumber),
    sizeof (STRING_REF)
    );
  ...
  SmbiosFldString (
    ...
    &(MemDevice->MemoryArrayLink.MemoryAssetTag),
    sizeof (STRING_REF)
    );
  ...
  SmbiosFldString (
    ...
    &(MemDevice->MemoryArrayLink.MemoryPartNumber),
    sizeof (STRING_REF)
    );
  ...
  SmbiosFldInterLink (
    ...
    16, // SMBIOS type 16
    &(MemDevice->MemoryArrayLink.MemoryArrayLink),
    &gEfiMemorySubClassGuid
    );
  ...
  // Memory Device Error Information (Type 18) Link
  //
  if (MemDevice->MemoryArrayLink.MemoryDeviceSize != 0) {
    SmbiosFldInterLink (
      StructureNode,
      (UINT16) OFFSET_OF (SMBIOS_TABLE_TYPE17, MemoryErrorInformationHandle),
      18, // SMBIOS type 18
      &(MemDevice->MemoryArrayLink.MemorySubArrayLink),
      &gEfiMemorySubClassGuid
      );
  }
  ...
  CopyMem (
    ...
    &(MemDevice->MemoryArrayLink.MemoryTotalWidth),
    2
    );
  ...
  CopyMem (
    ...
    &(MemDevice->MemoryArrayLink.MemoryDataWidth),
    2
    );
  ...
  // Device Size
  //
  MemoryDeviceSize          = MemDevice->MemoryArrayLink.MemoryDeviceSize;
  ...
  if (MemoryDeviceSize > 0xffff) {
    ...
    if (MemoryDeviceSize >= MEMORY_DEVICE_SIZE_MB_32GB) {
      MemoryExtendedSize = MemoryDeviceSize;
      MemoryDeviceSize   = 0x7fff;
    }
  }
  ...
  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x0c,
    &MemoryDeviceSize,
    2
    );
  ...
  //
  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x0e,
    &(MemDevice->MemoryArrayLink.MemoryFormFactor),
    1
    );
  ...
  //
  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x0f,
    &(MemDevice->MemoryArrayLink.MemoryDeviceSet),
    1
    );
  ...
  //
  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x12,
    &(MemDevice->MemoryArrayLink.MemoryType),
    1
    );
  ...
  //
  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x13,
    &(MemDevice->MemoryArrayLink.MemoryTypeDetail),
    2
    );
  ...
  //
  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x15,
    &(MemDevice->MemoryArrayLink.MemorySpeed),
    2
    );

  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x1B,
    &(MemDevice->MemoryArrayLink.MemoryAttributes),
    1
    );

  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x1c,
    &MemoryExtendedSize,
    4
    );

  CopyMem (
    (UINT8 *) StructureNode->Structure + 0x20,
    &(MemDevice->ConfiguredMemoryClockSpeed),
    2
    );
  ...
  )
{
  ...
  SetMem (
   (UINT8*)StructureNode->Structure + OFFSET_OF (SMBIOS_TABLE_TYPE6, BankConnections),
   1,
   0xFF
  );
  ...
  if (!FeaturePcdGet(PcdFrameworkCompatibilitySupport)) {
    ...
    if (FrameworkMemDevice->MemoryFormFactor == EfiMemoryFormFactorDimm ||
        FrameworkMemDevice->MemoryFormFactor == EfiMemoryFormFactorSodimm ||
  ...
  )
{
  ...
  if (MemoryController->NumberofMemorySlot == 0) {
    ...
    for (Count = 0, Link = MemoryController->MemoryModuleConfig; 
         ...
         Count++, Link++) {
      *(UINT8 *) ((UINT8 *) (StructureNode->Structure) + (0x0F + Count * sizeof(UINT16))) = 0xFF;
      SmbiosFldInterLink (
        StructureNode,
