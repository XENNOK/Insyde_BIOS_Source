  ...
  )
{
  ...
  UINTN                      BiosStartingAddressSegment;
  ...
  //
  // Correct SMBIOS Type 0 Bios Information - BiosStartingAddressSegment
  //
  BiosStartingAddressSegment = (UINTN)BiosInfo->BiosStartingAddress;
  BiosStartingAddressSegment &= 0x000FFFF0;
  BiosStartingAddressSegment >>= 4;
  //
  ...
  )
{
  ...
  ChassisSkuNumber = (STRING_REF*)(Ec + 1);
  ...
  *(UINT8*)((UINT8*)(StructureNode->Structure) +
  ...
  SmbiosFldString (
    StructureNode,
    0x16 + ((Ec->ChassisElementCount * Ec->ChassisElementRecordLength)),
    ChassisSkuNumber,
    2 // 64 * sizeof(CHAR16)
    );
  ...
  )
{
  ...
  //
  // Slot Segment Number/Slot Bus Number/Slot Dev/Func Number
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x0d,
    &Slot->SlotCharacteristics + 2,
    4
    );
...
{
  EFI_MISC_OEM_STRING_PLUS      *OemString;
  UINT8                         StringCount;
  UINT8                         Index;

  OemString = RecordData;
  //
  // OemString Count
  //
  StringCount = OemString->OemStringCount;
  CopyMem ((UINT8*)(StructureNode->Structure) + 0x4, &StringCount, 1);
  ...
  )
{
  ...
  NumberOfInstallableLanguages = (UINT8)(RecordDataSize / 2);
  CopyMem ((UINT8*)(StructureNode->Structure) + 0x4, &NumberOfInstallableLanguages, 1);

  for (Index = 0; Index < NumberOfInstallableLanguages; Index ++) {
    //
    // OnBoard Device Description
    //
    SmbiosFldStringEx (
  ...
  )
{
  ...
  STRING_REF                                     *LanguageStringPtr;
  UINT8                                          Index;
  UINT32                                         Type13Offset;
  UINT32                                         OriginLength;

  LanguageStringPtr = (STRING_REF *)((EFI_MISC_NUMBER_OF_INSTALLABLE_LANGUAGES *)RecordData + 1);
  ...
  *(UINT8 *) ((UINT8 *) (StructureNode->Structure) + OFFSET_OF (SMBIOS_TABLE_TYPE13, InstallableLanguages)) = (UINT8) (InstallableLanguage->NumberOfInstallableLanguages);
  ...
  OriginLength = StructureNode->StructureSize;
  Type13Offset = 0;
  //
  // BIOS Language Information table size is 0x15. The language string start from 0x16.
  //
  for (Index = 0; Index < InstallableLanguage->NumberOfInstallableLanguages; Index++) {
    SmbiosFldStringEx (
      StructureNode,
      &(LanguageStringPtr[Index])
      );
  }
  ...
  )
{
  ...
  EventLogTypeDescriptors = *(EFI_PHYSICAL_ADDRESS*)(SystemEventLog + 1);
  ...
  DataLength = (UINT8)(SystemEventLog->NumberOfSupportedLogType * SystemEventLog->LengthOfLogDescriptor);
  //
  // Data
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x17,
    (void *)(UINTN)EventLogTypeDescriptors,
    DataLength //+ 2
    );

  //
  // SMBIOS Type15 Length update
  //
  *(UINT8 *) ((UINT8 *) (StructureNode->Structure) + 0x01) = 0x17 + DataLength;
...
{
  EFI_MISC_VOLTAGE_PROBE_DESCRIPTION_PLUS *VoltageProbe;

  VoltageProbe = (EFI_MISC_VOLTAGE_PROBE_DESCRIPTION_PLUS *)RecordData;
...
{
  EFI_MISC_COOLING_DEVICE_TEMP_LINK_PLUS *CoolingDevice;

  CoolingDevice = (EFI_MISC_COOLING_DEVICE_TEMP_LINK_PLUS *)RecordData;

  //
  // Temperature Probe Handle
  //
  SetMem ((UINT8 *) (StructureNode->Structure) + 0x4, 2, 0xFF);
  ...
  CopyMem (
    ...
    &CoolingDevice->EfiMiscCoolingDevice.CoolingDeviceType,
    1
    );
  ...
  SmbiosFldInterLink (
    ...
    &CoolingDevice->EfiMiscCoolingDevice.CoolingDeviceTemperatureLink,
    &gEfiMiscSubClassGuid
    );
  ...
  CopyMem (
    ...
    &CoolingDevice->EfiMiscCoolingDevice.CoolingDeviceUnitGroup,
    1
    );
  ...
  CopyMem (
    ...
    &CoolingDevice->EfiMiscCoolingDevice.CoolingDeviceOemDefined,
    4
    );
  ...
  CopyMem (
    ...
    &CoolingDevice->EfiMiscCoolingDevice.CoolingDeviceNominalSpeed,
    2
    );
  ...
  )
{
  ...
  //
  SetMem (
    (UINT8 *) (StructureNode->Structure) + OFFSET_OF (SMBIOS_TABLE_TYPE39, InputVoltageProbeHandle),
    2,
    0xFF
    );
  SmbiosFldInterLink (
  ...
  //
  SetMem (
    (UINT8 *) (StructureNode->Structure) + OFFSET_OF (SMBIOS_TABLE_TYPE39, CoolingDeviceHandle),
    2,
    0xFF
    );
  SmbiosFldInterLink (
  ...
  //
  SetMem (
    (UINT8 *) (StructureNode->Structure) + OFFSET_OF (SMBIOS_TABLE_TYPE39, InputCurrentProbeHandle),
    2,
    0xFF
    );
  SmbiosFldInterLink (
...
{
  EFI_MISC_ADDITIONAL_INFORMATION       *AdditionalInfo;
  UINT8                                 NumberOfEntries;
  UINT8                                 *Ptr;
  UINT8                                 *EntriesOfTable;
  UINT32                                NewStructureSize;
  UINT32                                TotalEntriesSize;
  UINT8                                 Index;
  UINT16                                HandledEntriesSize;
  EFI_MISC_ADDITIONAL_INFORMATION_ENTRY *AdditionalInfoEntries;
  UINTN                                 *AddInfoAllocateAddr;
  EFI_STATUS                            Status;
  UINT32                                StructureBufferSize;
  UINTN                                 EntriesOffset;
  EFI_INTER_LINK_DATA                   ReferencedLink;

  Ptr                   = NULL;
  TotalEntriesSize      = 0;
  HandledEntriesSize    = 0;
  AdditionalInfo        = (EFI_MISC_ADDITIONAL_INFORMATION *)RecordData;
  NumberOfEntries       = AdditionalInfo->NumberOfAdditionalInformationEntries;
  AdditionalInfoEntries = (EFI_MISC_ADDITIONAL_INFORMATION_ENTRY *)((UINTN)(AdditionalInfo->AdditionalInfoEntriesAddr));
  AddInfoAllocateAddr   = (UINTN*)(UINTN)(AdditionalInfo->AdditionalInfoEntriesAddr);

  for (Index = 0; Index < NumberOfEntries; Index++) {
    TotalEntriesSize += AdditionalInfoEntries->EntryLength;
    AdditionalInfoEntries++;
  }

  //
  // Including 2 trailing zeros
  //
  NewStructureSize = 0x04 + sizeof (UINT8) + TotalEntriesSize + 2;
  StructureNode->StructureSize = NewStructureSize;
  StructureBufferSize = ((NewStructureSize - 1) / SMBIOS_MEM_UNIT_SIZE + 1) * SMBIOS_MEM_UNIT_SIZE;
  Ptr = AllocateZeroPool (StructureBufferSize);
  if (!Ptr) {
  ...
  CopyMem (
    Ptr,
    (UINT8 *) (StructureNode->Structure),
    0x04
    );

  gBS->FreePool (StructureNode->Structure);

  StructureNode->Structure = (EFI_SMBIOS_TABLE_HEADER *) Ptr;

  //
  // Update length field, exclude 2 trailing bytes of 0x00
  //
  StructureNode->Structure->Length = (UINT8)(StructureNode->StructureSize - 2);

  //
  // NumberOfAdditionalInformationEntries
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x04,
    &AdditionalInfo->NumberOfAdditionalInformationEntries,
    1
    );
  ...
  // Additional Entries
  //
  EntriesOfTable = (UINT8 *) (StructureNode->Structure) + 0x05;
  EntriesOffset = 0;
  AdditionalInfoEntries = (EFI_MISC_ADDITIONAL_INFORMATION_ENTRY *)((UINTN)(AdditionalInfo->AdditionalInfoEntriesAddr));
  for (Index = 0; Index < NumberOfEntries; Index++) {
    //
    ...
    CopyMem (
      EntriesOfTable + 0x00,
      &AdditionalInfoEntries->EntryLength,
      1
      );
    ...
    // Referenced Handle
    //
    ReferencedLink = AdditionalInfoEntries->ReferencedLink;
    SetMem ((UINT8 *) (StructureNode->Structure) + 0x05 + HandledEntriesSize + 0x01 , 2, 0xFF);
    SmbiosFldInterLink (
      StructureNode,
      0x05 + HandledEntriesSize + 0x01,
      AdditionalInfoEntries->ReferencedSmbiosType,  // Referenced SMBIOS type
      &ReferencedLink,
      &gEfiMiscSubClassGuid
      );
    ...
    CopyMem (
      EntriesOfTable + 0x03,
      &AdditionalInfoEntries->ReferencedOffset,
      1
      );
    ...
    SmbiosFldString (
      StructureNode,
      0x05 + HandledEntriesSize + 0x04,
      &(AdditionalInfoEntries->EntryString),
      2
      );

    EntriesOfTable = (UINT8 *) (StructureNode->Structure) + 0x05 + EntriesOffset;
    EntriesOffset = EntriesOffset + AdditionalInfoEntries->EntryLength;
    ...
    CopyMem (
      EntriesOfTable + 0x05,
      (VOID *)((UINTN)(AdditionalInfoEntries->ValueAddress)),
      AdditionalInfoEntries->EntryLength - 5
      );

    //
    // Next entry
    //
    HandledEntriesSize = HandledEntriesSize + (UINT16) (AdditionalInfoEntries->EntryLength);
    EntriesOfTable = EntriesOfTable + AdditionalInfoEntries->EntryLength;
    AdditionalInfoEntries++;
  }

  if (NumberOfEntries >= 1) {
    Status = gBS->FreePool (AddInfoAllocateAddr);
...
{
  EFI_MISC_ONBOARD_DEVICES_EXTENDED_INFORMATION *OnBoardDevicesInfo;

  OnBoardDevicesInfo = (EFI_MISC_ONBOARD_DEVICES_EXTENDED_INFORMATION *)RecordData;
  ...
  SmbiosFldString (
    StructureNode,
    0x04,
    &(OnBoardDevicesInfo->ReferenceDesignation),
    2 // 64 * sizeof(CHAR16)
    );
  ...
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x05,
    &OnBoardDevicesInfo->DeviceType,
    1
    );
  ...
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x06,
    &OnBoardDevicesInfo->DeviceTypeInstance,
    1
    );
  ...
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x07,
    &OnBoardDevicesInfo->SegmentGroupNum,
    2
    );
  ...
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x09,
    &OnBoardDevicesInfo->BusNum,
    1
    );
  ...
  //
  CopyMem (
    (UINT8 *) (StructureNode->Structure) + 0x0A,
    &OnBoardDevicesInfo->DevFuncNum,
    1
    );
...
/**
 Field Filling Function for Misc SubClass record type 136 - CRID

 @param [in, out] StructureNode  Smbios type structure node instance
 @param [in]   Offset           Offset for record data information in this type
 @param [in]   RecordData       Data buffer of record
 @param [in]   RecordDataSize   Record data size.

 @retval EFI_SUCCESS            successful
 @return Other Vals              something wrong

**/
EFI_STATUS
SmbiosFldMiscType136 (
  IN OUT  SMBIOS_STRUCTURE_NODE     *StructureNode,
  IN      UINT32                    Offset,
  IN      VOID                      *RecordData,
  IN      UINT32                    RecordDataSize
...
{
  EFI_MISC_COMPATIBLE_REVISION_ID    *CompatibleRevisionIDData;

  CompatibleRevisionIDData = ( EFI_MISC_COMPATIBLE_REVISION_ID * ) RecordData;


  //
  // Value for Enable / Disable CRID
  //
  CopyMem (
    ( UINT8 * )( StructureNode->Structure ) + 0x04,
    &CompatibleRevisionIDData->FieldValue,
    2
    );
  ...
  )
{
  ...
  if (StructureSize < RecordDataSize) {
    ...
    //
    SmbiosUpdateStructureBuffer (StructureNode, RecordData);
  } else {
    //
