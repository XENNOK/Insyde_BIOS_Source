  ...
  )
{
  ...
  while(TRUE) {
    ...
    for (Link = mStructureList.ForwardLink; Link != &mStructureList; Link = Link->ForwardLink) {
      ...
      if (Conversion->StructureLocatingMethod == BySubclassInstanceSubinstanceProducer) {
        ...
        if (CompareGuid (&(StructureNode->SubClass), &(RecordHeader->DataRecordGuid)) &&
          ...
          if ((Conversion->SmbiosType >= 0x80) && (Conversion->SmbiosType != 0x88)) {
    ...
    if (Link == &mStructureList || StructureNode == NULL) {
      ...
      //
      // The Length field excludes the TWO trailing zero byte that is counted
      // by StructureSize and SmbiosGetTypeMinimalLength()
      //
      if (Conversion->SmbiosType == 15) {
        StructureNode->StructureSize += ((EFI_MISC_SYSTEM_EVENT_LOG *) SrcData)->NumberOfSupportedLogType * 2;
      }
