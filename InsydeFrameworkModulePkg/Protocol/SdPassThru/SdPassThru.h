...
#ifndef _EFI_SD_PASS_THRU_PROTOCOL_H_
#define _EFI_SD_PASS_THRU_PROTOCOL_H_

#include "Tiano.h"

EFI_FORWARD_DECLARATION (EFI_SD_PASS_THRU);

//
//  EFI SdPassThru protocol
//
#define EFI_SD_PASS_THRU_PROTOCOL_GUID \
  {\
    0x593ea8f0, 0x201b, 0x4ade, 0xb4, 0xff, 0x44, 0x70, 0x78, 0x57, 0x57, 0x19 \
  }

typedef struct _EFI_SD_PASS_THRU_PROTOCOL   EFI_SD_PASS_THRU_PROTOCOL;
...
typedef
EFI_STATUS
(EFIAPI *EFI_SD_PASS_THRU_PASSTHRU) (
  IN EFI_SD_PASS_THRU_PROTOCOL *This,
  IN SD_CMD_PARAMETER          *CmdParameters,
  IN UINT8                     SlotNum,
  IN OUT VOID                  *DataBuffer,
  IN OUT UINT16                *SdResponse
)
/*++

Routine Description:

  Sends an SD command to an SD device that is attached to the SD controller.

Arguments:

  This                 - A pointer to the EFI_SD_PASS_THRU_PROTOCOL instance.
  Port                 - The port number of the SDHC device to send the command.
  PortMultiplierPort   - The port multiplier port number of the SDHC device to send the command.
                         If there is no port multiplier, then specify 0.
  Packet               - A pointer to the SDHC command to send to the SDHC device specified by Port and
                         PortMultiplierPort.
  Event                - If non-blocking I/O is not supported then Event is ignored, and blocking I/O is
                         ...
                         Event will be signaled when the SDHC command completes.
Returns:
...
typedef
EFI_STATUS
(EFIAPI *EFI_SD_PASS_THRU_GET_CARD_INFO) (
  IN  EFI_SD_PASS_THRU_PROTOCOL *This,
  IN  UINT8                     SlotNum,
...
Arguments:
  ...
  This                 - A pointer to the EFI_SD_PASS_THRU_PROTOCOL instance.
  DevicePath           - A pointer to the device path node that describes an SDHC device on the SDHC controller.
  Port                 - On return, points to the port number of an SDHC device on the SDHC controller.
  PortMultiplierPort   - On return, points to the port multiplier port number of an SDHC device on the SDHC controller.
...
typedef
EFI_STATUS
(EFIAPI *EFI_SD_PASS_THRU_RESET_SLOT) (
  IN EFI_SD_PASS_THRU_PROTOCOL  *This,
  IN UINT8                      SlotNum
)
/*++
...
Routine Description:
  ...
  Resets a specific port on the SDHC controller. This operation also resets all the SDHC devices connected to the port.

Arguments:

  This                 - A pointer to the EFI_SD_PASS_THRU_PROTOCOL instance.
  Port                 - The port number on the SDHC controller.
...
typedef struct _EFI_SD_PASS_THRU_PROTOCOL {
  EFI_SD_PASS_THRU_PASSTHRU          PassThru;
  EFI_SD_PASS_THRU_GET_CARD_INFO     GetCardInfo;
} EFI_SD_PASS_THRU_PROTOCOL;


extern EFI_GUID gEfiSdPassThruProtocolGuid;
