  ...
  )
{
  ...
  for (Dev = 0, HCNum = 0; Dev < 0x20; Dev ++) {
    for (Fun = 0; Fun < 8; Fun ++) {
      ...
      PciRead (EfiPciWidthUint32, EhciAddress + PLATFORM_USB20_RID, &ClassCode);
      if ((ClassCode & 0xFFFFFF00) == 0x0C032000) {
        PciRead (EfiPciWidthUint16, EhciAddress + PLATFORM_USB20_VID, &VenderID);
        //
        // Check vender ID
        //
...
--*/
{
  ...
  PciRead (EfiPciWidthUint32, PciAddress, &BarAddress);
  if (BarAddress != 0) {
  ...
  PciWrite (EfiPciWidthUint32, PciAddress, &DebugIoConfig.EhciBar);
...
--*/
{
  ...
  Address = ReadBAR ();
  Address += ((EFI_USB_CAP_REG*)Address)->CapLength;
  return  (EFI_USB_CTRL_REG *)Address;
}
...
--*/
{
  ...
  PciRead (EfiPciWidthUint16, PciAddress, &Offset);
  Address = ReadBAR () + (Offset & 0x1fff);
  return  (EFI_USB_DBG_REG *)Address;
}
...
--*/
{
  ...
  Address = ReadBAR ();
  PortNum = (((EFI_USB_CAP_REG*)Address)->HcsParams >> 20) & 0x0f;
  if (PortNum) PortNum --;
...
--*/
{
  ...
  if (EhciTable[EhciVender].VenderID == 0x10de) {
    ...
    PciRead (EfiPciWidthUint32, PciAddress, &Data);
    Data &= ~0x0000f000;
    Data |= (PortNum + 1) << 12;
    PciWrite (EfiPciWidthUint32, PciAddress, &Data);
  }
}
...
--*/
{
  ...
  PciWrite (EfiPciWidthUint16, PciAddress, &Data);
  //
  // Enable BME and MSE
  //
  Data = 0x06;
  PciAddress = GetEhciAddress (PLATFORM_USB20_PCICMD);
  PciWrite (EfiPciWidthUint8, PciAddress, &Data);
}
...
--*/
{
  ...
  PciAddress = GetEhciAddress(PLATFORM_USB20_MEMBASE);
  PciRead (EfiPciWidthUint32, PciAddress, &EhciBar);

  if (EhciBar != 0) return;

  EhciBar = DebugIoConfig.EhciBar;
  PciWrite (EfiPciWidthUint32, PciAddress, &EhciBar);
}
...
--*/
{
  ...
  while ((CtrlPtr->PortSC[UsbDebugPortNum] & EUSBPORTSC_CCS) != EUSBPORTSC_CCS) {
    Stall (1);
    if (!Index) {
      CpuIoWrite8 ((UINT16)DebugIoConfig.Config.DebugPort, (UINT8)TestPostCode);
      TestPostCode = ByteRotate (TestPostCode);
      Index = 500;
  ...
  //
  CpuIoWrite8((UINT16)DebugIoConfig.Config.DebugPort, 0xd1);
...
--*/
{
  ...
  UINTN                         StringLength;
  UINT8                         TempBuffer[8];
  volatile UINT32               Ticks, CurrentTick;
  volatile EFI_USB_CTRL_REG     *CtrlPtr;
  ...
  for (; *OutputString != 0; OutputString++) {
    ...
    if (StringLength == 8){
      ...
      Memzero (TempBuffer, 8);
      //
      // Wait for host to receive data from debug cable.
      //
      CurrentTick = CtrlPtr->Frindex;
      while ((CurrentTick + TIME_FRAMES) > 0x3FFF) CurrentTick = CtrlPtr->Frindex;
      while (Ticks < (CurrentTick + TIME_FRAMES)) Ticks = CtrlPtr->Frindex;
