...
//[-start-130719-IB07390109-remove]//
//EFI_SMM_SYSTEM_TABLE                  *mSmst;
//[-end-130719-IB07390109-remove]//

#include "CpuIA32.h"

ESAL_FWB_GLOBAL         *mFvbModuleGlobal;
FLASH_DEVICE                                *GlobalFlashOperationPtr;
...
//
// Note: In PEI stage, FLASH_BASE[FLASH_SIZE] has been setup by PeiBuildHobResourceDescriptor
//
VOID *mFlashBasePointer = (VOID*)((UINTN)FLASH_REGION_NVSTORAGE_SUBREGION_NV_VARIABLE_STORE_BASE);
...
--*/
{
  ...
  EfiConvertPointer (EFI_INTERNAL_POINTER, (VOID **) &mFlashBasePointer);

  //
  // Convert FdSupport library and GlobalFlashOperationPtr
  //
  FdSupportConvertPointer (&GlobalFlashOperationPtr);
...
--*/
{
  ...
  //
  // Get FdTableInfoHob to get the connected Flash device
  //
  ...
  if (GlobalFlashOperationPtr == NULL) {
    Status = FlashFdDetectDevice (NULL);
    ASSERT_EFI_ERROR (Status);
  }
  if (((UINTN)FdTableHobPtr->CurrentFdIndex != gCurrentFdIndex) || (FdTableHobPtr->CurrentFdId != gCurrentFdId)) {
    //
    // gCurrentFdIndex is just updated by FlashFdDetectDevice.
    // Update CurrentFdIndex of FdTableHob to the detected one.
    //
    FdTableHobPtr->CurrentFdIndex = (UINT8)gCurrentFdIndex;
    FdTableHobPtr->CurrentFdId    = gCurrentFdId;
  }
...
--*/
{
  ...
  if (GlobalFlashOperationPtr == NULL && !EfiAtRuntime ()) {
    FlashFdDetectDevice (NULL);
  }

  if (!EfiAtRuntime ()) OldTpl = gBS->RaiseTPL (EFI_TPL_NOTIFY);
  ...
  if (EFI_ERROR (Status)) {
    Status = GlobalFlashOperationPtr->Program(Dest, Src, NumBytes, WriteAddress);
  } else {
    Status = GlobalFlashOperationPtr->Program((UINT8 *)SpiAddress, Src, NumBytes, WriteAddress);
  }
#ifndef NO_FLUSH_NVSTORE_CACHE
  //
  ...
  if (mFlashBasePointer != NULL) {
    WBCacheModify (WriteAddress - FLASH_REGION_NVSTORAGE_SUBREGION_NV_VARIABLE_STORE_BASE + (UINTN)mFlashBasePointer, *NumBytes);
  }
#endif
  EnableFvbWrites (FALSE);
  EfiWbinvd ();
  if (!EfiAtRuntime ()) gBS->RestoreTPL (OldTpl);
...
Routine Description:
  Detect the kind of Flash device
...
//[-start-130719-IB07390109-modify]//
  POSTCODE (DXE_IDENTIFY_FLASH_DEVICE); //PostCode = 49, Identify Flash device

  Status = AutoDetectDevice (&GlobalFlashOperationPtr, NULL);
//[-end-130719-IB07390109-modify]//
  ...
  if (!EFI_ERROR (Status)) {
    if (Buffer != NULL) {
      EfiCopyMem (Buffer, GlobalFlashOperationPtr, sizeof (FLASH_DEVICE));
    }
  }
...
Routine Description:
  ...
  Set the memory attribute of BIOS region to EFI_MEMORY_RUNTIME
...
Returns:
  EFI_SUCCESS     - Set memory attribute successful
  Other           - Set memory attribute failed
...
--*/
{
  ...
  //
  BaseAddress = ((UINT64) FLASH_BASE) & (~EFI_PAGE_MASK);
  Length = (FLASH_SIZE + EFI_PAGE_SIZE - 1) & (~EFI_PAGE_MASK);
...
--*/
{
  ...
  if (EFI_ERROR (Status)) {
    Status = GlobalFlashOperationPtr->Erase (
                                        WriteAddress,
                                        LbaLength
                                        );
  } else {
...
#ifndef NO_FLUSH_NVSTORE_CACHE
  EfiCpuFlushCache (WriteAddress, LbaLength);
  if (mFlashBasePointer != NULL) {
    WBCacheModify (WriteAddress - FLASH_REGION_NVSTORAGE_SUBREGION_NV_VARIABLE_STORE_BASE + (UINTN)mFlashBasePointer, LbaLength);
  }
  ...
  )
{
  ...
  *WriteBack    = FALSE;
  ...
  if (FlashMapEntry->AreaType == EFI_FLASH_AREA_RESERVED_0A) {
    goto ProduceFv;
  }
#endif
  ...
  //
  if (!EfiCompareGuid (&FlashMapEntry->AreaTypeGuid, &gEfiSystemNvDataHobGuid)) {
    return Status;
  }
...
--*/
{
  ...
  BlockSize = 0;
#ifdef USE_FAST_CRISIS_RECOVERY
  GetHobBootMode (HobList, &BootMode);
  ...
  //
  // Recalculate the Fv header that maybe written back into flash area.
  //
  ...
  do {
    ...
    if (WriteEnabled) {
      ...
      if (WriteBack && (!WriteLocked)) {
        ...
        //
        //Calculate the Fd block size
        //
        if (GlobalFlashOperationPtr == NULL) {
          FlashFdDetectDevice (NULL);
        }
        BlockSize = (UINTN) GlobalFlashOperationPtr->DeviceInfo.BlockMap.BlockSize;
        BlockSize *= 256;
...
Arguments:
  ...
  CurrentBaseAddress  - GC_TODO: add argument description
  NewBaseAddress      - GC_TODO: add argument description
  WriteEnabled        - GC_TODO: add argument description
...
--*/
{
  *NewBaseAddress = CurrentBaseAddress;
  *WriteEnabled   = TRUE;
  return EFI_SUCCESS;
}
...
Arguments:
  ...
  LbaAddress  - GC_TODO: add argument description
