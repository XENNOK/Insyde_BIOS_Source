...
EFI_HII_DATABASE_PROTOCOL          *gBMHiiDatabase;
EFI_HII_STRING_PROTOCOL            *gBMHiiString;
EFI_FORM_BROWSER2_PROTOCOL         *gBMFormBrowser2;
EFI_HII_CONFIG_ROUTING_PROTOCOL    *gBMHiiConfigRouting;
...
--*/
{
  ...
  Status = gBS->LocateProtocol (&gEfiHiiDatabaseProtocolGuid, NULL, &gBMHiiDatabase);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = gBS->LocateProtocol (&gEfiHiiStringProtocolGuid, NULL, &gBMHiiString);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = gBS->LocateProtocol (&gEfiFormBrowser2ProtocolGuid, NULL, &gBMFormBrowser2);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = gBS->LocateProtocol (&gEfiHiiConfigRoutingProtocolGuid, NULL, &gBMHiiConfigRouting);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Status = gBMHiiDatabase->NewPackageList (
                             gBMHiiDatabase,
                             PackageList,
                             BmmCallbackInfo->BmmDriverHandle,
  ...
  //
  gBMHiiDatabase->RemovePackageList (gBMHiiDatabase, BmmCallbackInfo->BmmHiiHandle);
  gBMHiiDatabase->RemovePackageList (gBMHiiDatabase, BmmCallbackInfo->FeHiiHandle);
