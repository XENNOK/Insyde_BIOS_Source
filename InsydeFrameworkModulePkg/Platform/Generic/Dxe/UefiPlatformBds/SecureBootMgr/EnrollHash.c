...
EFI_TEXT_OUTPUT_STRING     mOriginalOutputString;
...
Arguments:
  ...
  BufferSize     - The image size
  IsSigned       - TRUE: It is a signed EFI image
                   FALSE: It isn't a signed EFI image.

Returns:

  TRUE                   - This image is EFI format image.
  FALSE                  - This image isn't EFI format image.
--*/
{
...
Arguments:
  ...
  HashAlg        - Hash algorithm type
  ImageDigest    - Pointer to EFI_HASH_OUTPUT instance.

Returns:

  EFI_INVALID_PARAMETER      - Any input parameter is invalid.
  EFI_UNSUPPORTED            - The input hash type is unsupported.
  EFI_SUCCESS                - add hash image to link list successful.
...
--*/
{
  ...
  HashLink     = EfiAllocateZeroPool (sizeof (HASH_LINK_LIST));
  ...
  EfiCopyMem (HashLink->Hash, ImageDigest->Sha256Hash, HashLink->HashSize);
  ...
  )
{
  ...
  ImageDigest = EfiAllocateZeroPool (sizeof (EFI_HASH_OUTPUT));
  ImageDigest->Sha256Hash = EfiAllocateZeroPool (sizeof (EFI_SHA256_HASH));
  ...
  //
  SectionHeader = (EFI_IMAGE_SECTION_HEADER *) EfiAllocateZeroPool (sizeof (EFI_IMAGE_SECTION_HEADER) * NtHeader.Pe32->FileHeader.NumberOfSections);
  ...
  for (Index = 0; Index < NtHeader.Pe32->FileHeader.NumberOfSections; Index++) {
    ...
    while ((Pos > 0) && (Section->PointerToRawData < SectionHeader[Pos - 1].PointerToRawData)) {
      EfiCopyMem (&SectionHeader[Pos], &SectionHeader[Pos - 1], sizeof (EFI_IMAGE_SECTION_HEADER));
      Pos--;
    }
    EfiCopyMem (&SectionHeader[Pos], Section, sizeof (EFI_IMAGE_SECTION_HEADER));
    Section += 1;
  }
...
Arguments:
  ...
  Buffer         - Pointer to the image buffer.
  BufferSize     - The image size.
  AuthData       - Pointer to the Authenticode Signature retrieved from signed image.
  AuthDataSize   - Size of the Authenticode Signature in bytes.
...
Returns:
  ...
  EFI_INVALID_PARAMETER       AuthData is NULL or AuthDataSize is 0.
--*/
{
  UINT8                     Index;
  ...
  for (Index = 0; Index < HASHALG_MAX; Index++) {
    ...
    if (EfiCompareMem (AuthData + 32, mHash[Index].OidValue, mHash[Index].OidLength) == 0) {
      break;
    }
  }
...
Returns:
  ...
  EFI_SUCCESS         - This image is EFI format image.
...
--*/
{
  ...
  if (WinCertificate->wCertificateType == WIN_CERT_TYPE_PKCS_SIGNED_DATA) {
    ...
    if (EfiCompareGuid(&WinCertUefiGuid->CertType, &gEfiCertPkcs7Guid) &&
        WinCertUefiGuid->Hdr.dwLength >= sizeof (WIN_CERTIFICATE) + sizeof (EFI_GUID)) {
      AuthData = WinCertUefiGuid->CertData;
      AuthDataSize = WinCertUefiGuid->Hdr.dwLength - sizeof (WIN_CERTIFICATE) - sizeof (EFI_GUID);
      SupportedSig = TRUE;
    }
  }
  ...
  )
{
  ...
  if (!IsEfiImage (Buffer, BufferSize, &IsSignedEfi)) {
    GetStringFromHandle (
      mSecureBootPrivate.HiiHandle,
...
--*/
{
  ...
  if ((NULL != CurrentListNode) && (NULL != CurrentListNode->Next)) {
    ...
    IfrLibNewString (CallbackData->HiiHandle, &(NextListNode->StringToken), L" ");
...
--*/
{
  ...
  MenuEntry = EfiAllocateZeroPool (sizeof (ENROLL_MENU_ENTRY));
  if (NULL == MenuEntry) {
    return MenuEntry;
  }
...
--*/
{
  mUpdateData.BufferSize = EM_UPDATE_DATA_SIZE;
  mUpdateData.Data = EfiAllocateZeroPool (EM_UPDATE_DATA_SIZE);
}
...
Arguments:
  ...
  CallbackData    - Pointer to SECURE_BOOT_MANAGER_CALLBACK_DATA instance
