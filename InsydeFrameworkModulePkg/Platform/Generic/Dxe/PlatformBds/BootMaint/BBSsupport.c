...
--*/
{
  ...
  AsciiToUnicodeSize(TempString, BufSize, *BootString);
  if (TempString != NULL) {
...
--*/
{
  ...
  while (Index < BootOrderSize / sizeof (UINT16)) {
    ...
    if (EfiCompareMem (&LocalBbsTable[BbsIndex], BbsEntry, sizeof (BBS_TABLE)) == 0) {
      BbsEntryMatch = TRUE;
    }
    ...
    if (!((LocalBbsTable[BbsIndex].BootPriority == BBS_IGNORE_ENTRY) ||
          ...
          DescStringMatch && BbsEntryMatch) {
      Index++;
      continue;
    }
    if (ChipsetLibServices->NeedToOverWriteVar ()) {
      OverwriteBootVar = FALSE;

      for (TempIndex = TempStart; TempIndex < BbsCount; TempIndex++) {
        //
        // skip unused BBS table to make sure doesn't overwrite the boot option with null description string
        //
        if (((LocalBbsTable[TempIndex].BootPriority == BBS_IGNORE_ENTRY) ||
             (LocalBbsTable[TempIndex].BootPriority == BBS_DO_NOT_BOOT_FROM) ||
             (LocalBbsTable[TempIndex].BootPriority == BBS_LOWEST_PRIORITY))) {
           continue;
        }
        BdsBuildLegacyDevNameString (
          &LocalBbsTable[TempIndex],
          LocalHddInfo,
          TempIndex,
          sizeof(BootDesc),
          BootDesc
        ...
        if ((ChipsetLibServices->DifferentSizeStrCmp (BootDesc, (UINT16*)(BootOptionVar + sizeof (UINT32) + sizeof (UINT16))) == 0) &&
          ...
          //
          if (!CheckLegacyBootVar ((UINT16) TempIndex, BootDesc, LocalBbsTable[TempIndex].DeviceType)) {
            UpdateBootVar (&LocalBbsTable[TempIndex],BootDesc,BootOption,TempIndex);
