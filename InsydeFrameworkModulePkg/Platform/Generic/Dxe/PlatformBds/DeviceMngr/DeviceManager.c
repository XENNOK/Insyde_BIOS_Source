...
--*/
{
  ...
  EFI_FORM_BROWSER_PROTOCOL  *DMBrowser;
  ...
  // a target to display
  //
  if (gDMCallbackKey != 0 && gDMCallbackKey < 0x2000) {
    BootDeviceMngrMenuResetRequired = FALSE;
    Status = DMBrowser->SendForm (
                          DMBrowser,
                          TRUE,                               // Use the database
                          (EFI_HII_HANDLE *) &gDMCallbackKey, // The HII Handle
                          1,
                          NULL,
