...
--*/
{
  ...
  //
  Index = 0;
  MsgString = GetStringById (STRING_TOKEN (STR_RECOVERY_FLASH_MSG));
  ...
  FlashInfoStringArray[1] = GetStringById (STRING_TOKEN (STR_RECOVERY_FLASH_NO));
  ...
  while (TRUE) {
    ...
    if ((Key.ScanCode != SCAN_ESC) && (Key.UnicodeChar != CHAR_CARRIAGE_RETURN)) {
      ...
      if (Key.ScanCode == SCAN_UP || Key.ScanCode == SCAN_DOWN) {
        //
        // It only has two items. No matter press SCAN_UP or SCAN_DOWN,
        // the item index will be changed
        //
        Index = Index == 0 ? 1 : 0;
      }
    } else {
