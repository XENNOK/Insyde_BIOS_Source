...
UpdateNewForm (
  ...
  IN EFI_FORM_LABEL                            BootDevLabel,
  IN UINT8                                     *Location,
...
--*/
{
  ...
  DevicePath = NULL;
  DevicePath = SetupUtilityLibGetVariableAndSize (
                 L"PortableWorkSpace",
  ...
  for (Index = 0; Index < EfiBootDeviceNum; Index++) {
    ...
    for (Index2 = 0; Index2 < HwBootDeviceCount; Index2++) {
      if ((DevicePathType (UpdateEfiBootString[Index].DevicePath) == MESSAGING_DEVICE_PATH) &&
          (DevicePathSubType (UpdateEfiBootString[Index].DevicePath) == MSG_USB_CLASS_DP)) {
        if (DevicePath == NULL) {
          HwDeviceName = NoDeviceStr;
        } else if (CompareBlockIoDevicePath (HwBootDeviceInfo[Index2].BlockIoDevicePath, DevicePath)) {
          HwDeviceName = HwBootDeviceInfo[Index2].HwDeviceName;
        }
      } else if (CompareBlockIoDevicePath (
                   HwBootDeviceInfo[Index2].BlockIoDevicePath,
                   UpdateEfiBootString[Index].DevicePath)) {
        HwDeviceName = HwBootDeviceInfo[Index2].HwDeviceName;
      }
      if (HwDeviceName != NULL) {
        String = EfiLibAllocateZeroPool (
                   EfiStrSize (UpdateEfiBootString[Index].pString) + EfiStrSize (L" (") +
    ...
    if (HwBootDeviceCount == 0 &&
        DevicePath == NULL &&
        (DevicePathType (UpdateEfiBootString[Index].DevicePath) == MESSAGING_DEVICE_PATH) &&
        (DevicePathSubType (UpdateEfiBootString[Index].DevicePath) == MSG_USB_CLASS_DP)) {
      HwDeviceName = NoDeviceStr;
      String = EfiLibAllocateZeroPool (
                 EfiStrSize (UpdateEfiBootString[Index].pString) + EfiStrSize (L" (") +
...
--*/
{
  ...
  UINT8                                 *LocationBootTypeOrder;
  UINT8                                 *LocationAdvance;
  UINT8                                 *LocationLegacyAdv;
  UINT8                                 *LocationEfiBoot;
  UINT8                                 *LocationDevices[MAX_BOOT_ORDER_NUMBER];
  ...
  //
  UpdateBootTypeOrderData = EfiLibAllocateZeroPool (0x1000);
  UpdateAdvData           = EfiLibAllocateZeroPool (0x1000);
  UpdateAdvLegacyData     = EfiLibAllocateZeroPool (0x1000);
  UpdateEfiData           = EfiLibAllocateZeroPool (0x1000);
  //
  // UpdateXXX is for Hii update data
  //
  UpdateBootTypeOrderData->DataCount = LegacyBootTypeOrderCount;
  UpdateAdvData->DataCount           = AdvBootDeviceNum;
  UpdateAdvLegacyData->DataCount     = LegacyBootDeviceNum;
  UpdateEfiData->DataCount           = EfiBootDeviceNum;
  for (Index = 0; Index < LegacyBootTypeOrderCount; Index++) {
    UpdateBootDeviceData[TempBootType[Index]] = EfiLibAllocateZeroPool (0x1000);
    UpdateBootDeviceData[TempBootType[Index]]->DataCount = (UINT16) mBbsTypeTable[TempBootType[Index]].DeviceTypeCount;
  }

  //
  // assign data address by type
  //
  LocationBootTypeOrder = (UINT8 *) &UpdateBootTypeOrderData->Data;
  LocationAdvance       = (UINT8 *) &UpdateAdvData->Data;
  LocationLegacyAdv     = (UINT8 *) &UpdateAdvLegacyData->Data;
  LocationEfiBoot       = (UINT8 *) &UpdateEfiData->Data;

  for (Index = 0; Index < LegacyBootTypeOrderCount; Index++) {
    LocationDevices[TempBootType[Index]] = (UINT8 *) &UpdateBootDeviceData[TempBootType[Index]]->Data;
  }

  IfrFlag = EFI_IFR_FLAG_MANUFACTURING;
  //
  ...
  for (Index = 0; Index < (UINT16) LegacyBootTypeOrderCount; Index++, LastToken = 0) {
    ...
    CreateTextOpCode (
      ...
      0,
      IfrFlag,
      (UINT16) (KEY_BOOT_TYPE_ORDER_BASE + Index),
      LocationBootTypeOrder
      );

    LocationBootTypeOrder = LocationBootTypeOrder + ((EFI_IFR_OP_HEADER *) LocationBootTypeOrder)->Length;
  }
  //
  ...
  //
  // update adv boot form
  //
  LastToken = 0;

  if (AdvBootDeviceNum != 0) {
    SetupVariable->NoAdvBootDev = 1;
  }

  BootDevLabel = BOOT_DEVICE_LABEL;
  DevKeyBase = KEY_BOOT_DEVICE_BASE;
  UpdateNewForm (
    HiiHandle,
    IfrFlag,
    &LastToken,
  ...
  SUCInfo->AdvanceTokenRecord = LastToken;
  ...
  for (Index = 0; Index < LegacyBootTypeOrderCount; Index++) {
    ...
    UpdateNewForm (
      ...
      LocationDevices[TempBootType[Index]],
  ...
  if (EfiBootDeviceNum != 0) {
    CreateEfiOption (SetupVariable, HiiHandle);
  }
  ...
  )
{
  ...
  SecondUpdateStr = SetupUtilityLibGetTokenString (
                      HiiHandle,
                      SecondToken
                      );

  FirstUpdateStr = SetupUtilityLibGetTokenString (
                     HiiHandle,
                     FirstToken
                     );
  Hii->NewString(
...
Routine Description:
  ...
  In SCU, create EFI option which contain suppressif condition.

Arguments:

  SetupVariable  -  Pointer to the SYSTEM_CONFIGURATION instance
  HiiHandle      -  Handle on which the form resides

Returns:

  EFI_SUCCESS    -  Create successful
  else           -  Create fail
...
--*/
{
  ...
  SETUP_UTILITY_BROWSER_DATA            *SuBrowser;
  EFI_HII_PROTOCOL                      *Hii;
  UINT8                                 *LocationEfiOption;
  EFI_HII_UPDATE_DATA                   *UpdateEfiOptionData;
  EFI_IFR_SUPPRESS                      SuppressifData;
  EFI_IFR_EQ_ID_VAL                     IdequalData;
  EFI_IFR_OR                            OrData;
  EFI_IFR_END_IF                        EndifData;
  ...
  UpdateEfiOptionData = EfiLibAllocateZeroPool (0x1000);
  UpdateEfiOptionData->DataCount = 6;
  LocationEfiOption = (UINT8 *) &UpdateEfiOptionData->Data;

  //
  // Sequentially create OpCodes of suppressif, idequal, goto (EFI option) and endif.
  //
  SuppressifData.Header.OpCode = EFI_IFR_SUPPRESS_IF_OP;
  SuppressifData.Header.Length = 3;
  SuppressifData.Flags = 0;
  EfiCopyMem (LocationEfiOption, &SuppressifData, SuppressifData.Header.Length);
  LocationEfiOption += SuppressifData.Header.Length;

  IdequalData.Header.OpCode = EFI_IFR_EQ_ID_VAL_OP;
  IdequalData.Header.Length = 7;
  IdequalData.QuestionId = (UINT16) ((&SetupVariable->BootType - (UINT8 *) SetupVariable) / sizeof (UINT8));
  IdequalData.Width = 1;
  IdequalData.Value = 1;
  EfiCopyMem (LocationEfiOption, &IdequalData, IdequalData.Header.Length);
  LocationEfiOption += IdequalData.Header.Length;

  IdequalData.Header.OpCode = EFI_IFR_EQ_ID_VAL_OP;
  IdequalData.Header.Length = 7;
  IdequalData.QuestionId = (UINT16) ((&SetupVariable->NoEfiBootDev - (UINT8 *) SetupVariable) / sizeof (UINT8));
  IdequalData.Width = 1;
  IdequalData.Value = 0;
  EfiCopyMem (LocationEfiOption, &IdequalData, IdequalData.Header.Length);
  LocationEfiOption += IdequalData.Header.Length;

  OrData.Header.OpCode = EFI_IFR_OR_OP;
  OrData.Header.Length = 2;
  EfiCopyMem (LocationEfiOption, &OrData, OrData.Header.Length);
  LocationEfiOption += OrData.Header.Length;

  CreateGotoOpCode (
    BOOT_DEVICE_EFI_FORM_ID,
    (STRING_REF) STRING_TOKEN (STR_EFI),
    (STRING_REF) STRING_TOKEN (STR_EFI_HELP),
    0,
    0,
    LocationEfiOption
    );
  LocationEfiOption += sizeof (EFI_IFR_REF);

  EndifData.Header.OpCode = EFI_IFR_END_IF_OP;
  EndifData.Header.Length = 2;
  EfiCopyMem (LocationEfiOption, &EndifData, EndifData.Header.Length);
  LocationEfiOption += EndifData.Header.Length;

  Status = Hii->UpdateForm (
                  Hii,
                  HiiHandle,
                  (EFI_FORM_LABEL) UPDATE_EFI_OPTION_LABEL,
                  TRUE,
                  UpdateEfiOptionData
                  );

  gBS->FreePool (UpdateEfiOptionData);
  return Status;
}
