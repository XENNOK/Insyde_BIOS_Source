...
--*/
{
  ...
  UpdateLangItem (This, (UINT8 *) (&(MyIfrNVData->Language)));
    ...
    MyIfrNVData->SupervisorFlag = SUCInfo->SCBuffer->SupervisorFlag;
    MyIfrNVData->UserFlag       = SUCInfo->SCBuffer->UserFlag;
...
--*/
{
  ...
  MyIfrNVData->Language = ((SYSTEM_CONFIGURATION *)Interface->MyIfrNVData)->Language;
  LangStr = (CHAR8 *) SetupUtilityLibGetVariableAndSize (L"CustomPlatformLang", &gEfiGenericVariableGuid, &BufferSize);
  if (LangStr != NULL) {
  ...
  MyIfrNVData->Tpm2DeviceOk  = ((SYSTEM_CONFIGURATION *)Interface->MyIfrNVData)->Tpm2DeviceOk;
