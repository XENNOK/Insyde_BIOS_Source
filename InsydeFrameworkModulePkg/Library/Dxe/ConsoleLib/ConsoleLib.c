...
Returns:
  ...
  EFI_SUCCESS            The width of the character is found and successfully returned.
  EFI_UNSUPPORTED        The character is not found.       
...
--*/
{
  ...
  if (UnicodeChar <= mUnicodeWidthTable[0].WChar) {
    *GlyphWidth = mUnicodeWidthTable[0].Width;
    return EFI_SUCCESS;
  }
  ...
  do {
    ...
    for (; (String[Index] != NARROW_CHAR) && (String[Index] != WIDE_CHAR) && (String[Index] != 0); Index++) {
      ...
      if (IncrementValue == 1) {
        ...
        //
        Status = GetGlyphWidth (String[Index], &GlyphWidth);
        if (!EFI_ERROR(Status) && GlyphWidth == 2) {
          Count++;
        }
      }
    }
...
Returns:
  ...
  EFI_SUCCESS      The length of the string is successfully returned.
  EFI_UNSUPPORTED  Some of the characters is not supported by our glyph.
