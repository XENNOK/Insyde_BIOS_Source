...
//[-start-140512-IB08400265-modify]//
  DebugMessageBuffer = 0xFFFFFFFF;
  TableSize = PcdGet32(PcdStoreDebugMsgSize);
  ...
  //
  if ((PcdGet32(PcdStoreDebugMsgEntryPei) == 0xFFFFFFFF) || (PcdGet32(PcdStoreDebugMsgEntryPei) == 0)) {
    //
  ...
  PeiDebugMessageHead = (PEI_DEBUG_MESSAGE_HEAD_STRUCTURE *)(UINTN)PcdGet32(PcdStoreDebugMsgEntryPei);
  PeiDebugMessageSize = PeiDebugMessageHead->AdmgEndAddr - PeiDebugMessageHead->AdmgStartAddr;
  if (PeiDebugMessageSize != 0) {
    if (PeiDebugMessageSize <= DebugMessageHead->AdmgSize) {
      //
      // The size of DEBUG messages in PEI phase in less than the DXE size.
      //
      CopyMem ((VOID*)(UINTN)(DebugMessageHead->AdmgStartAddr), (VOID*)(UINTN)(PeiDebugMessageHead->AdmgStartAddr), PeiDebugMessageSize);
      DebugMessageHead->AdmgEndAddr += (UINT32)PeiDebugMessageSize; 
    } else {
      //
      // The size of DEBUG messages in PEI phase in less than the DXE size.
      //
      CopyMem ((VOID*)(UINTN)(DebugMessageHead->AdmgStartAddr), (VOID*)(UINTN)(PeiDebugMessageHead->AdmgStartAddr), DebugMessageHead->AdmgSize);
      DebugMessageHead->AdmgEndAddr += (UINT32)DebugMessageHead->AdmgSize; 
    }
  }
//[-end-140512-IB08400265-modify]//
  ...
  )
{
   ...
   PcdSet32 (PcdStoreDebugMsgMemAddress, 0xFFFFFFFF); 
...
//[-end-140512-IB08400265-add]//
  ...
  if (EFI_ERROR (Status)) {
    ...
    //
    // Register callback for un-registering the worker function.
    //
//[-start-140512-IB08400265-modify]//
