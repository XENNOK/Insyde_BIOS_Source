  ...
  )
{
  ...
  for (CacheLineTestSize = *CurrentCacheLine; CacheLineTestSize >= CACHE_LINE_SIZE_MIN; CacheLineTestSize >>= 1) {
  ...
  )
{
  ...
  for (Device = 0; Device <= PCI_MAX_DEVICE; Device++) {
    for (Func = 0; Func <= PCI_MAX_FUNC; Func++) {
      if (*(CacheLineSizeTable + (Device * (PCI_MAX_FUNC + 1) + Func)) >= CurrentCacheLine) {
        Address = EFI_PCI_ADDRESS (Bus, Device, Func, PCI_CACHELINE_SIZE_OFFSET);
        PciRootBridgeIo->Pci.Write (
                               PciRootBridgeIo,
                               EfiPciWidthUint8,
  ...
  )
{
  ...
  UINT8                             MinCacheLine;
  UINT8                             *CacheLineSizeTable;

  MinCacheLine       = CACHE_LINE_SIZE_MAX;
  CacheLineSizeTable = AllocateZeroPool (sizeof(UINT8)*(PCI_MAX_DEVICE+1)*(PCI_MAX_FUNC+1));
