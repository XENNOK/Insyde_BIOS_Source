  ...
  )
{
  ...
  I2cMaster->StartRequest ( 
               ...
               (Event == NULL) ? NULL : I2cHostContext->I2cEvent,
  ...
  )
{
  ...
  if(!IsListEmpty (EntryHeader)) {
    I2cHostRequestEnable (I2cHostContext, TRUE);
  }
  ...
  )
{
  ...
  Event = (AsyncRequest) ? I2cHostContext->I2cBusConfigurationEvent : NULL;
  //
  ...
  if (I2cHostContext->I2cBusConfiguration != I2cBusConfiguration ) {
    ...
    if (AsyncRequest) {
      //
      // No necessary to execute subsequence code upon async request as it
      // is executed by EnableI2cBusConfiguration completed notification
      //
      return;
    }
  }
  ...
  I2cHostI2cBusConfigurationAvailable (Event, I2cHostContext);
  if (AsyncRequest) {
    //
