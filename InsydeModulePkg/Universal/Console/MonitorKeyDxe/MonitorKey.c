  ...
  )
{
  ...
  do {
    ...
    ZeroMem(&KeyPad, sizeof(EFI_KEY_DATA));
      ...
      if (mPrivate->HotKeyList[Index].Keyattribute.ShiftKey ||
        ...
        for (Count = 0; EfiKeyShiftStateTable[Count][0] != 0xFF; Count++) {
          ...
          if (KeyData.Key.ScanCode != SCAN_NULL && KeyData.Key.UnicodeChar != 0x00) {
            CopyMem(&KeyPad, &KeyData, sizeof(EFI_KEY_DATA));
            IsKeyPad = TRUE;
            //
            // KeyData only register ScanCode, KeyPad only register UnicodeChar.
            //
          ...
          if (IsKeyPad) {
            ...
            // Restore UnicodeChar
            //
            KeyData.Key.UnicodeChar = KeyPad.Key.UnicodeChar;
            IsKeyPad = FALSE;
          }
