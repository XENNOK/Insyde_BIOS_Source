...
typedef struct {
  ...
  EFI_HANDLE                  HandleInDXE;
  EFI_HANDLE                  HandleInSMM;
  H2O_SD_HOST_IO_PROTOCOL     SdHostIo;
  EFI_PHYSICAL_ADDRESS        BaseMemAddr;
  UINT8                       SlotNum;
  UINT8                       HostSpecVer;
  HOST_CAPABILITY             HostCapability;
  UINTN                       BaseClockInKHz;
  BOOLEAN                     IsAutoStopCmd;
  BOOLEAN                     IsDdrMode;
  //
  // Data Transfer (move from CardData here)
  //
  UINT8                      *RawBufferPointer;   //DmaBufferAddr 
  UINT8                      *AlignedBuffer; 
  UINT8                      *DescriptorTableAddr;
  BOOLEAN                    SmmInstalled;
  UINTN                      Bus;
  UINTN                      Dev;
  UINTN                      Fun; 
