...
Device(IRDV) {  // COM Port

  Name (_HID, EISAID("PNP0510"))  // PnP Device ID
  Name(_UID, "SModuleIr")      // Generic ID for IrDA

  //*****************************************************
  // Method _STA:  Return Status
  //*****************************************************
  Method (_STA, 0, NotSerialized) { // Return Status of the IR
  ...
  Method (_DIS, 0, NotSerialized) { // Disable Method
    ...
    // Set Logical Device to select the IR
    Store (SIO_IR, CR07)
  ...
  Method (_CRS, 0, NotSerialized) {
    ...
    Name (BUF0, ResourceTemplate() {
      IO (Decode16, 0x310, 0x310, 0x08, 0x08)
      IRQ (Level, ActiveLow, Shared) {0x0B}
    })
    //
  ...
  Method(_PRS,0) {   // Possible Resource    
    ...
    Name(BUF0,ResourceTemplate() {
      ...
      {
       IO(Decode16,0x310,0x310,0x01,0x08)
  ...
  Method (_SRS, 1, NotSerialized) {    // Set Resource Settings
    ...
    // Set Logical Device to select IR
    Store(SIO_IR, CR07)
