...
/*grep Argc and Argv Start*/
  ...
  if ((Argc == 1) || 
    ...
    Print (L"Argc %d , Argv[1][0] %c",Argc,Argv[1][0]);
    return	EFI_SUCCESS;
  } else if ((Argc >= 4) &&
             (Argv[Argc - 3][0] != '-')) {
    Print(L"Error parameter!! \n -h to viewing parameter Usage\n");
    Print (L"Argc %d, Argv[Argc - 2][0]\n",Argc,Argv[Argc - 2][0]);
    return	EFI_SUCCESS;
  }
...
//    Print (L"%d-Argv = %s ",ArgIndex,Argv[ArgIndex]);
