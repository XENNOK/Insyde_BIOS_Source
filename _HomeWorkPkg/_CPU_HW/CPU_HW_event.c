    ...
    if ((HKey.Key.ScanCode != 0 ) &&
      ...
      break;
       ...
       break;
               ...
               (HKey.Key.UnicodeChar >= 'a' && HKey.Key.UnicodeChar <= 'd') ||
               (HKey.Key.UnicodeChar >= 'A' && HKey.Key.UnicodeChar <= 'D') ||
               (HKey.Key.UnicodeChar == 'r') || (HKey.Key.UnicodeChar =='R') ||
               (HKey.Key.UnicodeChar == 'w') || (HKey.Key.UnicodeChar =='W'))
              ) {
      if (HKey.Key.UnicodeChar >= '0' && HKey.Key.UnicodeChar <= '9') {
         SelectedItem = (UINT8) (HKey.Key.UnicodeChar - '0');
         break;
      } else if (HKey.Key.UnicodeChar >= 'a' && HKey.Key.UnicodeChar <= 'd') {
        SelectedItem = (UINT8) ((HKey.Key.UnicodeChar - 'a') + 10); 
        break;
      } else if (HKey.Key.UnicodeChar >= 'A' && HKey.Key.UnicodeChar <= 'D') {
        SelectedItem = (UINT8) ((HKey.Key.UnicodeChar - 'A') + 10);
        break;
      } else if ((HKey.Key.UnicodeChar == 'r') || (HKey.Key.UnicodeChar =='R') ) {
         SelectedItem = 23;
         break;
      } else if ((HKey.Key.UnicodeChar == 'w') || (HKey.Key.UnicodeChar =='W') ) {
         SelectedItem = 24;
         break;
      }
	  } else if ((HKey.KeyState.KeyShiftState & EFI_SHIFT_STATE_VALID) &&
  	  ((HKey.KeyState.KeyShiftState & EFI_LEFT_ALT_PRESSED) ||
  	   (HKey.KeyState.KeyShiftState & EFI_RIGHT_ALT_PRESSED)) &&
  	  (HKey.Key.ScanCode == 0) && (HKey.Key.UnicodeChar >= '0' && HKey.Key.UnicodeChar <= '9')) {
  	  SelectedItem = (UINT8) (HKey.Key.UnicodeChar - '0') + 14;
      break;
	  }
  }
