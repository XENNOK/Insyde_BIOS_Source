/** @file
  ...
  Implementation file for SPS HECI functionality in DXE Phase.
  Note: Only for SPS.
...
SPS_HECI_PROTOCOL            *mHeciDxe = NULL;

/**
  Retrieves the HECI protocol for internal ues.
**/
EFI_STATUS
SpsInternalGetDxeHeciProtocol (
  VOID
  )
{
  EFI_STATUS            Status;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[DXE SPS HECI LIB] ERROR: Locate SPS HECI Protocol => (%r)\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  Status = SpsInternalGetDxeHeciProtocol ();
  if ((EFI_ERROR (Status)) || (mHeciDxe == NULL)) {
    return EFI_NOT_STARTED;
  }

  Status = mHeciDxe->SendwACK (
                       Message,
                       Length,
                       RecLength, 
                       HostAddress, 
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[DXE SPS HECI LIB] ERROR: Send HECI Message and wait for response => (%r)\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  Status = mHeciDxe->ReadMsg (
                       Blocking,
                       (UINT32 *) MessageBody,
                       Length
                       );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[DXE SPS HECI LIB] ERROR: Read HECI Message => (%r)\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[DXE SPS HECI LIB] ERROR: Send HECI Message => (%r)\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  Status = mHeciDxe->InitHeci ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[DXE SPS HECI LIB] ERROR: HECI initialize => (%r)\n", Status));
    return Status;
  }
