/** @file
  ...
  Server Platform Services(SPS) ME Firmware Upgrade DXE driver
...
#include "SpsMeFwUpgradeDxe.h"

#define SPS_DEBUG_NAME                 "[DXE SPS ME FW UPGRADE] "
...
SPS_ME_UPGRADE_CONTEXT                 mDxeSpsMeUpgradeCtx;
  ...
  )
{
  ...
  do {
    Status = FlashErase (LbaWriteAddress, *NumBytes);
    RetryCount++;
  ...
  )
{
  ...
  do {
    Status = FlashRead (Buffer, (UINT8 *) (WriteAddress + Offset), *NumBytes);
    RetryCount++;
  } while ((EFI_ERROR (Status)) && (RetryCount < 100));
  ...
  )
{
  ...
  SPS_DEBUG ((DEBUG_INFO, "   Buffer  = 0x%08x\n", Buffer));
  do {
    Status = FlashProgram ((UINT8 *)Offset, Buffer, NumBytes, Offset);
    RetryCount++;
  } while ((EFI_ERROR (Status)) && (RetryCount < 100));
  ...
  )
{
  ...
  do {
    ...
    if (!EFI_ERROR (Status) && (TempNonceInfo.Result == SPS_HMRFPO_LOCK_SUCCESS)) {
      mDxeSpsMeUpgradeCtx.MeRegionEnabled = FALSE;
      break;
    }
    RetryCount++;
  ...
  )
{
  ...
  do {
    ...
    if (!EFI_ERROR (Status) && (TempNonceInfo.Result == SPS_HMRFPO_ENABLE_SUCCESS)) {
      mDxeSpsMeUpgradeCtx.MeRegionEnabled = TRUE;
      break;
    }
    RetryCount++;
  ...
  )
{
  ...
  if (((WriteTargetAddr + WriteTargetSize) > mSpsMeMapInfo.MeRegionBase) && (WriteTargetAddr <= mSpsMeMapInfo.MeRegionLimit)) {
    if (!mDxeSpsMeUpgradeCtx.MeRegionEnabled) {
      //
      ...
      if (TempNonceInfo.Result == SPS_HMRFPO_STS_LOCKED) {
        Status = SpsUpgradeHmrfpoEnable (mDxeSpsMeUpgradeCtx.NonceInfo->Nonce, &TempNonceInfo);
  ...
  )
{
  ...
  SPS_DEBUG ((DEBUG_INFO, "\n\n"));
  SPS_DEBUG ((DEBUG_INFO, SPS_DEBUG_NAME"INFO: %a(%d) WriteTargetAddr => 0x%08x\n", __FUNCTION__, __LINE__, WriteTargetAddr));
  ...
  )
{
  ...
  Status = InitializeContext (&mDxeSpsMeUpgradeCtx);
  ...
  Status = gBS->InstallProtocolInterface (
                  ...
                  &(mDxeSpsMeUpgradeCtx.SpsMeUpgrade)
                  );
  SPS_DEBUG ((DEBUG_INFO, SPS_DEBUG_NAME"INFO: Install SPS ME Upgrade Support protocol (%r)\n", Status));
