  ...
  )
{
  ...
  Status = gBS->HandleProtocol (Handle, &gEfiDiskInfoProtocolGuid, &DiskInfo);
  if (EFI_ERROR(Status))
    return DT_UNKNOW;
  ...
  )
{
  ...
  Status = gBS->HandleProtocol (FsHandle, &gEfiSimpleFileSystemProtocolGuid, &FileSystem);
  if (EFI_ERROR(Status)) 
    return NULL;
...
//[-end-140625-IB05080432-add]//
  Status = RootFs->GetInfo (RootFs, &gEfiFileSystemInfoGuid, &Size, VolumeInfo);
  if (Status == EFI_SUCCESS) {
    ValueToString (VolumeInfo->VolumeSize, 32, Buffer, VTS_LEFT_ALIGN);
  ...
  )
{
  ...
  Status = gBS->HandleProtocol (Handle, &gEfiBlockIoProtocolGuid, &BlkIo);
  if (EFI_ERROR(Status)){
    return FALSE;
  }
  ...
  )
{
  ...
  ConOutDevPath = (EFI_DEVICE_PATH_PROTOCOL *)GetVariable (L"ConOut", &gEfiGlobalVariableGuid);
  ...
  )
{
  ...
  if (mActiveVgaGop == NULL) {
    ...
    gBS->HandleProtocol (VgaHandle, &gEfiGraphicsOutputProtocolGuid, &mActiveVgaGop);
  }
