  ...
  )
{
  ...
  Status = gBS->HandleProtocol (Handle, &gEfiSimpleTextOutProtocolGuid, &TextOut);
  if (EFI_ERROR(Status))
    return Status;
  ...
  mHookTextOut.CharBuffer = AllocateZeroPool (MaxProduct * sizeof(UINT8) + MaxProduct * sizeof(CHAR16));
