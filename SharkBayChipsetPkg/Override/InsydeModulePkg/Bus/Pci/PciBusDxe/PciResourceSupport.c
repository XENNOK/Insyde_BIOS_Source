...
//[-start-140521-IB05400527-add]//
    //
    // WA for IGD (0:2:0) and ME (0:16:0)
    // Current IGD could not access 64-bit address, so degrade it's resources from PMem64 to PMem32
    // Current ME could not access 64-bit address and cause HECI command fail, so degrade it's resources from PMem64 to PMem32
    //
    if (((Temp->BusNumber == 0) && (Temp->DeviceNumber == 0x02) && (Temp->FunctionNumber == 0)) || 
        ((Temp->BusNumber == 0) && (Temp->DeviceNumber == 0x16) && (Temp->FunctionNumber == 0))) {
