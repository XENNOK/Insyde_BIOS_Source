/** @file
  ...
  Heci driver core. For SMM Phase, determines the HECI device and initializes it.
...
{
  volatile SPS_HECI_HOST_CONTROL_REGISTER   *HeciRegHCsrPtr;

  HeciRegHCsrPtr = (SPS_HECI_HOST_CONTROL_REGISTER *) a;
...
/**
  ...
  in its PCI cfg space are setup properly and that the local mSmmHeciContext
  ...
  )
{
  ...
  ///
  if (SpsHeciPciRead32 (R_HECIMBAR) & 0x4) {
    mSmmHeciContext->HeciMBAR = (UINT64) SpsHeciPciRead32 (R_HECIMBAR) & 0xFFFFFFF0;
    mSmmHeciContext->HeciMBAR |= ((UINT64) SpsHeciPciRead32 (R_HECIMBAR + 4)) << 32;
  } else {
    mSmmHeciContext->HeciMBAR = (UINT64) SpsHeciPciRead32 (R_HECIMBAR) & 0xFFFFFFF0;
  }
  ///
  ...
  ///
  if ((SpsHeciPciRead8 (PCI_COMMAND_OFFSET) & (EFI_PCI_COMMAND_MEMORY_SPACE | EFI_PCI_COMMAND_BUS_MASTER)) !=
  ...
  DEBUG ((EFI_D_INFO, "[SMM HECI] INFO: SMM HECI BAR = 0x%08x\n", mSmmHeciContext->HeciMBAR));

  return mSmmHeciContext->HeciMBAR;
}
...
{
  SPS_HECI_HOST_CONTROL_REGISTER            HeciRegHCsr;
  volatile SPS_HECI_HOST_CONTROL_REGISTER   *HeciRegHCsrPtr;
  EFI_STATUS                                Status;
  SPS_HECI_FWS_REGISTER                     MeFirmwareStatus;
  ...
  do {
    ...
    ///
    mSmmHeciContext->DeviceInfo = SpsHeciPciRead16 (PCI_DEVICE_ID_OFFSET);

    ///
    /// Check for HECI-1 PCI device availability
    ///
    if (mSmmHeciContext->DeviceInfo == 0xFFFF) {
      Status = EFI_DEVICE_ERROR;
      break;
    }

    MeFirmwareStatus.ul = SpsHeciPciRead32 (R_FWSTATE);
    ...
    if (MeFirmwareStatus.r.ErrorCode) {
      ...
      if (MeFirmwareStatus.r.ErrorCode == SPS_ME_ERROR_CODE_UNKNOWN || MeFirmwareStatus.r.ErrorCode == SPS_ME_ERROR_CODE_IMAGE_FAILURE) {
        ///
    ...
    ///
    if ((MeFirmwareStatus.r.MeOperationMode == SPS_ME_OPERATION_MODE_SECOVR_JMPR) ||
        (MeFirmwareStatus.r.MeOperationMode == SPS_ME_OPERATION_MODE_DEBUG)
        ) {
    ...
    ///
    mSmmHeciContext->RevisionInfo = SpsHeciPciRead8 (PCI_REVISION_ID_OFFSET);
    ...
    if (SpsHeciPciRead32 (R_HECIMBAR) & 0x4) {
      mSmmHeciContext->HeciMBAR = (((UINT64) SpsHeciPciRead32 (R_HECIMBAR + 4) << 32) |
                                    (UINT64) SpsHeciPciRead32 (R_HECIMBAR)) & 0xFFFFFFF0;
    } else {
      mSmmHeciContext->HeciMBAR = (UINT64) SpsHeciPciRead32 (R_HECIMBAR) & 0xFFFFFFF0;
    }
    ///
    ...
    /// to a useable value
    ///
    mHeciMBAR = mSmmHeciContext->HeciMBAR;

    ///
    /// Load temporary address for HECI_MBAR if one is not assigned
    ///
    if (mSmmHeciContext->HeciMBAR == 0) {
    ...
    ///     c) Set H_IG
    ///
    HeciRegHCsrPtr  = (VOID *) (UINTN) (mSmmHeciContext->HeciMBAR + H_CSR);
  ...
  )
{
  UINT32                          TimerStart;
  UINT32                          TimerEnd;
  SPS_HECI_ME_CONTROL_REGISTER    HeciRegMeCsrHa;
  ...
  ///
  StartTimer (&TimerStart, &TimerEnd, SPS_HECI_INIT_TIMEOUT);
  HeciRegMeCsrHa.ul = MMIOREADDWORD (mHeciMBAR + ME_CSR_HA);
  ...
  )
{
  ...
  /// Init Host & ME CSR
  ///
  HeciRegHCsr.ul    = MMIOREADDWORD (mHeciMBAR + H_CSR);
  HeciRegMeCsrHa.ul = MMIOREADDWORD (mHeciMBAR + ME_CSR_HA);
  ...
  )
{
  ...
  ///
  mHeciMBAR = CheckAndFixHeciForAccess ();
  ...
  if (HeciRegHCsr.r.H_RDY == 0) {
    ...
    HeciRegHCsr.r.H_RDY = 1;
    HeciRegHCsr.r.H_IG  = 1;
    MMIOWRITEDWORD (mHeciMBAR + H_CSR, HeciRegHCsr.ul);
  }
  ...
  )
{
  ...
  ///
  if (MeResetWait (SPS_HECI_INIT_TIMEOUT) != EFI_SUCCESS) {
    return EFI_TIMEOUT;
  }
  ...
  )
{
  ...
  ///
  ///  VOLATILE SPS_HECI_HOST_CONTROL_REGISTER *HeciRegHCsrPtr     = (VOID*)(mSmmHeciContext->HeciMBAR + H_CSR);
  ///  VOLATILE SPS_HECI_ME_CONTROL_REGISTER   *HeciRegMeCsrHaPtr  = (VOID*)(mSmmHeciContext->HeciMBAR + ME_CSR_HA);
  ///  VOLATILE UINT32                         *HeciRegMeCbrwPtr   = (VOID*)(mSmmHeciContext->HeciMBAR + ME_CB_RW);
  ///
  ...
  ///
  StartTimer (&TimerStart, &TimerEnd, SPS_HECI_READ_TIMEOUT);
  ...
  while (!GotMessage) {
    ...
    if (FilledSlots (HeciRegMeCsrHa.r.ME_CBRP_HRA, HeciRegMeCsrHa.r.ME_CBWP_HRA) > 0) {
      ...
      ///
      MessageHeader->Data = MMIOREADDWORD (mHeciMBAR + ME_CB_RW);
      ...
      ///
      HeciRegMeCsrHa.ul = MMIOREADDWORD (mHeciMBAR + ME_CSR_HA);
      if ((MessageHeader->Fields.Length + sizeof (SPS_HECI_MESSAGE_HEADER)) > (HeciRegMeCsrHa.r.ME_CBD_HRA * 4)) {
        *Length = 0;
        return EFI_DEVICE_ERROR;
      }
      ///
      ...
      if ((MessageHeader->Fields.Length) <= *Length) {
        ...
        ///
        StartTimer (&TimerStart1, &TimerEnd1, SPS_HECI_READ_TIMEOUT);
        ...
        for (i = 0; i < LengthInDwords; i++) {
          MessageData[i] = MMIOREADDWORD (mHeciMBAR + ME_CB_RW);
        }
        ///
...
{
  SPS_HECI_MESSAGE_HEADER         PacketHeader;
  ...
  do {
    ...
    ///
    StartTimer (&timer_start, &timer_end, SPS_HECI_READ_TIMEOUT);
  ...
  )
{
  ...
  UINT32                          SendLength;
  UINT32                          CurrentLength;
  SPS_HECI_MESSAGE_HEADER         MessageHeader;
  ...
  )
{
  ...
  /// Start timeout counter
  ///
  StartTimer (&timer_start, &timer_end, SPS_HECI_SEND_TIMEOUT);
  ...
  ///
  MMIOWRITEDWORD (mHeciMBAR + H_CB_WW, MessageHeader->Data);

  ///
  /// Write Message Body
  ///
  for (i = 0; i < LengthInDwords; i++) {
    MMIOWRITEDWORD (mHeciMBAR + H_CB_WW, MessageData[i]);
  }
  ///
  /// Set Interrupt Generate bit
  ///
  ...
  )
{
  ...
  for (RetryCount = 0; RetryCount < SPS_HECI_MAX_RETRY; RetryCount++) {
    ///
    /// Read Message
    ///
  ...
  )
{
  ...
  if (MeFirmwareStatus.r.CurrentState == SPS_ME_STATE_NORMAL && MeFirmwareStatus.r.ErrorCode == SPS_ME_ERROR_CODE_NO_ERROR) {
    *MeStatus = SPS_ME_READY;
  } else if (MeFirmwareStatus.r.CurrentState == SPS_ME_STATE_RECOVERY) {
    *MeStatus = SPS_ME_IN_RECOVERY_MODE;
  } else if (MeFirmwareStatus.r.CurrentState == SPS_ME_STATE_INIT) {
    *MeStatus = SPS_ME_INITIALIZING;
  } else if (MeFirmwareStatus.r.CurrentState == SPS_ME_STATE_DISABLE_WAIT) {
    *MeStatus = SPS_ME_DISABLE_WAIT;
  } else if (MeFirmwareStatus.r.CurrentState == SPS_ME_STATE_TRANSITION) {
    *MeStatus = SPS_ME_TRANSITION;
  } else {
    *MeStatus = SPS_ME_NOT_READY;
  }

  if (MeFirmwareStatus.r.FwUpdateInprogress) {
    *MeStatus |= SPS_ME_FW_UPDATES_IN_PROGRESS;
  }

  if (MeFirmwareStatus.r.FwInitComplete == SPS_ME_FIRMWARE_COMPLETED) {
    *MeStatus |= SPS_ME_FW_INIT_COMPLETE;
  }

  if (MeFirmwareStatus.r.MeBootOptionsPresent == SPS_ME_BOOT_OPTIONS_PRESENT) {
    *MeStatus |= SPS_ME_FW_BOOT_OPTIONS_PRESENT;
  }
  ...
  )
{
  ...
  switch (MeFirmwareStatus.r.MeOperationMode) {
    case SPS_ME_OPERATION_MODE_NORMAL:
    case SPS_ME_OPERATION_MODE_SPS:
      *MeMode = SPS_ME_MODE_NORMAL;
      break;

    case SPS_ME_OPERATION_MODE_DEBUG:
      *MeMode = SPS_ME_MODE_DEBUG;
      break;

    case SPS_ME_OPERATION_MODE_SOFT_TEMP_DISABLE:
      *MeMode = SPS_ME_MODE_TEMP_DISABLED;
      break;

    case SPS_ME_OPERATION_MODE_SECOVR_JMPR:
    case SPS_ME_OPERATION_MODE_SECOVR_HECI_MSG:
      *MeMode = SPS_ME_MODE_SECOVER;
      break;

    default:
      *MeMode = SPS_ME_MODE_FAILED;
  }
