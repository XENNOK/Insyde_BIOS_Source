...
///
extern HECI_INSTANCE  *mDxeHeciContext;
...
/**
  ...
  in its PCI cfg space are setup properly and that the local mDxeHeciContext
  ...
  )
{
  ...
  if (SpsHeciPciRead32 (R_HECIMBAR) & 0x4) {
    mDxeHeciContext->HeciMBAR = (UINT64) SpsHeciPciRead32 (R_HECIMBAR) & 0xFFFFFFF0;
    mDxeHeciContext->HeciMBAR |= ((UINT64) SpsHeciPciRead32 (R_HECIMBAR + 4)) << 32;
  } else {
    mDxeHeciContext->HeciMBAR = (UINT64) SpsHeciPciRead32 (R_HECIMBAR) & 0xFFFFFFF0;
  }
  ///
  ...
  DEBUG ((EFI_D_INFO, "[DXE HECI] INFO: DXE HECI BAR = 0x%0lx\n", mDxeHeciContext->HeciMBAR));

  return mDxeHeciContext->HeciMBAR;
}
  ...
  )
{
  ...
  do {
    ...
    ///
    mDxeHeciContext->DeviceInfo = SpsHeciPciRead16 (PCI_DEVICE_ID_OFFSET);

    ///
    /// Check for HECI-1 PCI device availability
    ///
    if (mDxeHeciContext->DeviceInfo == 0xFFFF) {
    ...
    ///
    mDxeHeciContext->RevisionInfo = SpsHeciPciRead8 (PCI_REVISION_ID_OFFSET);
    ...
    /// to a useable value
    ///
    mHeciMBAR = mDxeHeciContext->HeciMBAR;

    ///
    /// Load temporary address for HECI_MBAR if one is not assigned
    ///
    if (mDxeHeciContext->HeciMBAR == 0) {
    ...
    ///     c) Set H_IG
    ///
    HeciRegHCsrPtr  = (VOID *) (UINTN) (mDxeHeciContext->HeciMBAR + H_CSR);
  ...
  )
{
  ...
  ///
  ///  VOLATILE SPS_HECI_HOST_CONTROL_REGISTER *HeciRegHCsrPtr     = (VOID*)(mDxeHeciContext->HeciMBAR + H_CSR);
  ///  VOLATILE SPS_HECI_ME_CONTROL_REGISTER   *HeciRegMeCsrHaPtr  = (VOID*)(mDxeHeciContext->HeciMBAR + ME_CSR_HA);
  ///  VOLATILE UINT32                         *HeciRegMeCbrwPtr   = (VOID*)(mDxeHeciContext->HeciMBAR + ME_CB_RW);
  ///
