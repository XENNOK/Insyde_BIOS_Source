...

{
  ...
  BOOLEAN                             CREnable = FALSE;
  BOOLEAN                             CRFailSave = FALSE;
  EFI_STATUS                          CRStatus;
  ...
  CRStatus = gBS->LocateProtocol (&gEfiCRPolicyProtocolGuid, NULL, (VOID **)&CrPolicy);
  if(CRStatus == EFI_SUCCESS && 
     CrPolicy->CRSerialDevData.CRDevice[IsaIo->ResourceList->Device.UID].Type == ISA_SERIAL_DEVICE
     ) {
    CREnable   = CrPolicy->CRSerialDevData.CRDevice[IsaIo->ResourceList->Device.UID].CrDevAttr.PortEnable;
    ...
    for (Index = 0; Index < EntryCount; Index++) {
      if ((OpenInfoBuffer[Index].Attributes & EFI_OPEN_PROTOCOL_BY_CHILD_CONTROLLER) != 0) {
        ...
        if (!EFI_ERROR (Status)) {
          ...
          if (CRFailSave == TRUE && CREnable == TRUE) {
            IsaSerialFailSaveBaudRate(SerialDevice);
          }  else {
...
/**
  This function for Console Redirection FailSaveBaudRate feature.
  Function will base on current UART working baudrate to update 
  devicepath
